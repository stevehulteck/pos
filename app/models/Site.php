...
class Site extends CI_Model
{
    ...
    public function syncProductVariantQty($product_id, $warehouse_id, $option_id) {
    ...
    private function getVariantBalanceQuantity($variant_id, $warehouse_id = NULL) {
        $this->db->select('SUM(COALESCE(quantity_balance, 0)) as stock', False);
    ...
    public function calculateCost($product_id, $warehouse_id, $net_unit_price, $unit_price, $quantity, $product_name, $option_id, $item_quantity) {
        ...
        foreach ($pis as $pi) {
            ...
            if (!empty($pi) && $balance_qty <= $quantity && $quantity > 0) {
                ...
                if ($pi->quantity_balance >= $quantity && $quantity > 0) {
                    if(count($_SESSION['quantity_balance']) > 0) {
                        if($_SESSION['quantity_balance'][$product_id] > 0) {
                            
                            $balance_qty = $_SESSION['quantity_balance'][$product_id] - $quantity;
                            $_SESSION['quantity_balance'][$product_id] = $balance_qty;
                        } else {
                            //$qty = $this->getWarehouseProductItems($product_id, $warehouse_id);
                            //if($qty == $pi->quantity_balance) { $qty = $pi->quantity_balance; } else { $qty = $qty; }
                            $balance_qty = $pi->quantity_balance - $quantity;
                            //$balance_qty = $qty - $quantity;
                            $_SESSION['quantity_balance'][$product_id] = $balance_qty;
                        }
                    } else {
                        if($_SESSION['quantity_balance'][$product_id] > 0) {
        ...
        //echo $option_id; 
        //print_r($_SESSION['quantity_balance']); 
        if ($quantity > 0) {
            $this->session->set_flashdata('error', sprintf(lang("quantity_out_of_stock_for_%s"), ($pi->product_name ? $pi->product_name : $product_name)));
            redirect($_SERVER["HTTP_REFERER"]);
        }
    ...
    public function getWarehouseProductItems($product_id, $warehouse_id) {
    ...
    public function costing($items) {
        ...
        $cost = array();
        $_SESSION['quantity_balance'] = array();
            ...
            foreach ($sale_items as $item) {
                if ($item->product_type == 'standard') {
                    ...
                    if (isset($item->option_id) && !empty($item->option_id)) {
                        ...
                        $this->syncProductVariantQty($item->product_id, $item->warehouse_id, $item->option_id);
                    } else {
                        $this->syncProductQty($item->product_id, $item->warehouse_id);
                    }
                } elseif ($item->product_type == 'combo') {
