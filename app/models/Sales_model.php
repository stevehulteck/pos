...
class Sales_model extends CI_Model
{
    ...
    public function getProductNames($term, $warehouse_id, $limit = 5)
    {
        ...
        $this->db->select('products.*, FWP.quantity as quantity, categories.id as category_id, categories.name as category_name, product_variants.name as variant_name, product_variants.id as variant_id, product_variants.price as variant_price', FALSE)
        ...
        if ($this->Settings->overselling) {
            ...
            $this->db->where("({$this->db->dbprefix('products')}.code LIKE '%" . $term . "%' OR  concat({$this->db->dbprefix('products')}.code, REPLACE({$this->db->dbprefix('product_variants')}.name.'*','')) LIKE '%" . $term . "%' OR  concat({$this->db->dbprefix('products')}.code, REPLACE({$this->db->dbprefix('product_variants')}.code,'*','')) LIKE '%" . $term . "%')");
        } else {
            $this->db->where("FWP.warehouse_id = '" . $warehouse_id . "' AND "
                ...
                ."({$this->db->dbprefix('products')}.code LIKE '%" . $term . "%' OR  concat({$this->db->dbprefix('products')}.code, REPLACE({$this->db->dbprefix('product_variants')}.name,'*','')) LIKE '%" . $term . "%' OR  concat({$this->db->dbprefix('products')}.code, REPLACE({$this->db->dbprefix('product_variants')}.code,'*','')) LIKE '%" . $term . "%')");
        }
        $this->db->limit($limit);
    ...
    {
        $wpv = "( SELECT option_id, warehouse_id, quantity from {$this->db->dbprefix('warehouses_products_variants')} ) FWPV";
        $this->db->select('product_variants.id as id, product_variants.name as name, product_variants.price as price, product_variants.quantity as total_quantity', FALSE) //, FWPV.quantity as quantity
            ->join($wpv, 'FWPV.option_id=product_variants.id', 'left')
            //->join('warehouses', 'warehouses.id=product_variants.warehouse_id', 'left')
            ->where('product_variants.product_id', $product_id)
            ->where('FWPV.warehouse_id', $warehouse_id)
    ...
    public function updatePurchaseItem($id, $qty, $sale_item_id, $product_id = NULL, $warehouse_id = NULL, $option_id = NULL)
    {
        if ($id) {
			...
			//echo $id."<br>"; echo $qty."<br>"; echo $sale_item_id."<br>"; echo $product_id."<br>"; echo $warehouse_id."<br>"; echo $option_id."<br>";
