...
class Pos_model extends CI_Model
{
	...
	public function changeposchk($sid) {
		$this->db->where('id', $sid);
		$query = $this->db->get("sales");
		$result = $query->row();

		if($result->pos_check_sale== 1) {
			$data = array('pos_check_sale'=>0);
			$this->db->update('sales', $data, array('id' => $sid));
		} else {
			$data = array('pos_check_sale'=>1);
			$this->db->update('sales', $data, array('id' => $sid));
		}
		return true;
	}
    ...
    public function getRegisterMpSales($date, $user_id = NULL)
    {
        if (!$date) {
            $date = $this->session->userdata('register_open_time');
        }
        ...
        $this->db->select('COUNT(' . $this->db->dbprefix('payments') . '.id) as total_mpesa, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( amount, 0 ) ) AS paid', FALSE)
            ->join('sales', 'sales.id=payments.sale_id', 'left')
