...
class Reports extends MY_Controller
{
    ...
    function getProductsReport($pdf = NULL, $xls = NULL)
    {
        ...
        $pp = "( SELECT product_id, SUM(CASE WHEN pi.purchase_id IS NOT NULL THEN quantity ELSE 0 END) as purchasedQty, SUM(quantity_balance) as balacneQty, SUM( unit_cost * quantity_balance ) balacneValue, SUM( unit_cost * (CASE WHEN pi.purchase_id IS NOT NULL THEN quantity ELSE 0 END) ) totalPurchase from {$this->db->dbprefix('purchase_items')} pi LEFT JOIN {$this->db->dbprefix('purchases')} p on p.id = pi.purchase_id ";
    ...
    function poscheck()
    {
        $this->sma->checkPermissions('poscheck');
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['users'] = $this->reports_model->getStaff();
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('Pos_Check_Report')));
        $meta = array('page_title' => lang('Pos_Check_Report'), 'bc' => $bc);
        $this->page_construct('reports/poscheck', $meta, $this->data);
    }
    
    function invoicecheck()
    {
        $this->sma->checkPermissions('invoicecheck');
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['users'] = $this->reports_model->getStaff();
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('Invoice_Check_Report')));
        $meta = array('page_title' => lang('Invoice_Check_Report'), 'bc' => $bc);
        $this->page_construct('reports/invoicecheck', $meta, $this->data);
    }
    ...
    function getSalesReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select("date, IF(pos=1,reference_no,REPLACE(reference_no,'SALE','INVOICE')) as reference_no, biller, customer, GROUP_CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name) as pname, GROUP_CONCAT(" . $this->db->dbprefix('sale_items') . ".unit_price) as price, GROUP_CONCAT(" . $this->db->dbprefix('sale_items') . ".unit_quantity) as qty, GROUP_CONCAT(" . $this->db->dbprefix('product_variants') . ".name) as oname, GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, ' (', " . $this->db->dbprefix('sale_items') . ".quantity, ')') SEPARATOR '\n') as iname, grand_total, paid, payment_status", FALSE)
                ->from('sales')
            ...
            if (!empty($data)) {
                ...
                if ($pdf) {
                    //print_r($data); exit;
                    $this->sma->checkPermissions();
                    $this->data['biller'] = $data[0]->biller;
                    $this->data['customer'] = $data[0]->customer;
                    $this->data['fromdate'] = $data[count($data)-1]->date;
                    $this->data['todate'] = $data[0]->date;
                    ...
                    $html = $this->load->view($this->theme . 'reports/customer_pdf', $this->data, true);
                    //echo $html; exit;
            ...
            $si1 = "";
            $si = "SELECT {$this->db->dbprefix('sale_items')}.sale_id, {$this->db->dbprefix('sale_items')}.product_id, {$this->db->dbprefix('sale_items')}.serial_no, GROUP_CONCAT(IF(({$this->db->dbprefix('sale_items')}.option_id), (CONCAT({$this->db->dbprefix('sale_items')}.product_name, '__', CONCAT({$this->db->dbprefix('product_variants')}.name),'##',{$this->db->dbprefix('sale_items')}.unit_quantity)), (CONCAT({$this->db->dbprefix('sale_items')}.product_name,'__',{$this->db->dbprefix('sale_items')}.unit_quantity) ))SEPARATOR '___') as item_nane from {$this->db->dbprefix('sale_items')} LEFT JOIN {$this->db->dbprefix('product_variants')} ON {$this->db->dbprefix('product_variants')}.id = {$this->db->dbprefix('sale_items')}.option_id ";
            if ($product) {
                $si .= " WHERE {$this->db->dbprefix('sale_items')}.product_id = ".$product;
            }
            $si .= " GROUP BY {$this->db->dbprefix('sale_items')}.sale_id";
            
            $si1 = "( ".$si." ) FSI";
            $this->load->library('datatables');
            $this->datatables
                ->select("DATE_FORMAT(date, '%Y-%m-%d %T') as date, IF(pos=1,reference_no,REPLACE(reference_no,'SALE','INVOICE')) as reference_no, biller, customer, FSI.item_nane as iname, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.id as id", FALSE)
                ->from('sales')
                ->join($si1, 'FSI.sale_id=sales.id', 'left')
                ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left');
                //->group_by('sales.id');
    ...
    function getPosSalesReport($pdf = NULL, $xls = NULL)
    {
        $this->sma->checkPermissions('sales', TRUE);
        $product = $this->input->get('product') ? $this->input->get('product') : NULL;
        $user = $this->input->get('user') ? $this->input->get('user') : NULL;
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select("date, IF(pos=1,reference_no,REPLACE(reference_no,'SALE','INVOICE')) as reference_no, biller, customer, GROUP_CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name) as pname, GROUP_CONCAT(" . $this->db->dbprefix('sale_items') . ".unit_price) as price, GROUP_CONCAT(" . $this->db->dbprefix('sale_items') . ".quantity) as qty, GROUP_CONCAT(" . $this->db->dbprefix('product_variants') . ".name) as oname, GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, ' (', " . $this->db->dbprefix('sale_items') . ".quantity, ')') SEPARATOR '\n') as iname, grand_total, paid, payment_status", FALSE)
                ->from('sales')
                ->join('sale_items', 'sale_items.sale_id=sales.id', 'left')
                ->join('product_variants', 'product_variants.id=sale_items.option_id','left')    
                ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left')
                ->where('sales.pos_check_sale','1')
                ->group_by('sales.id')
                ->order_by('sales.date desc');
            ...
            if (!empty($data)) {
                ...
                if ($pdf) {
                    ...
                    $this->data['main_data'] = $data;
                    $name =  "sales_report.pdf";
                    $html = $this->load->view($this->theme . 'reports/pos_customer_pdf', $this->data, true);
                    //echo $html; exit;
                    $this->sma->generate_pdf($html, $name, false);
                }
                if ($xls) {
    ...
    function getInvoiceSalesReport($pdf = NULL, $xls = NULL)
    {
        $this->sma->checkPermissions('sales', TRUE);
        $product = $this->input->get('product') ? $this->input->get('product') : NULL;
        $user = $this->input->get('user') ? $this->input->get('user') : NULL;
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ...
                ->join('product_variants', 'product_variants.id=sale_items.option_id','left')    
                ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left')
                ->where('sales.invoice_check_sale','1')
                ->group_by('sales.id')
                ->order_by('sales.date desc');
            ...
            if (!empty($data)) {
                ...
                if ($pdf) {
                    ...
                    $this->data['main_data'] = $data;
                    $name =  "sales_report.pdf";
                    $html = $this->load->view($this->theme . 'reports/invoice_customer_pdf', $this->data, true);
                    //echo $html; exit;
                    $this->sma->generate_pdf($html, $name, false);
                }
                if ($xls) {
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("DATE_FORMAT(date, '%Y-%m-%d %T') as date, REPLACE(reference_no,'SALE','INVOICE') as reference_no, biller, customer, FSI.item_nane as iname, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.id as id", FALSE)
                ->from('sales')
                ->join($si, 'FSI.sale_id=sales.id', 'left')
                ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left')
                ->where('sales.invoice_check_sale','1');
    ...
    function getRrgisterlogs($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select("date, closed_at, CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ', " . $this->db->dbprefix('users') . ".last_name, ' (', users.email, ')') as user, cash_in_hand, total_mpesa, total_cheques, total_cash, total_mpesa_submitted, total_cheques_submitted,total_cash_submitted, note", FALSE)
                ->from("pos_register")
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('E1', lang('mpesa'));
                $this->excel->getActiveSheet()->SetCellValue('F1', lang('cheques'));
                $this->excel->getActiveSheet()->SetCellValue('G1', lang('total_cash'));
                $this->excel->getActiveSheet()->SetCellValue('H1', lang('mpesa_submitted'));
                ...
                foreach ($data as $data_row) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->total_mpesa);
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('H' . $row, $data_row->total_mpesa_submitted);
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("date, closed_at, CONCAT(" . $this->db->dbprefix('users') . ".first_name, ' ', " . $this->db->dbprefix('users') . ".last_name, '<br>', " . $this->db->dbprefix('users') . ".email) as user, cash_in_hand, CONCAT(total_mpesa, ' (', total_mpesa_submitted, ')'), CONCAT(total_cheques, ' (', total_cheques_submitted, ')'), CONCAT(total_cash, ' (', total_cash_submitted, ')'), note", FALSE)
                ->from("pos_register")
                ->join('users', 'users.id=pos_register.user_id', 'left');
