...
class Sales extends MY_Controller
{
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        $detail_link2 = anchor('sales/viewreceipt/$1', '<i class="fa fa-file-text-o"></i> ' . lang('view_receipt'));
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, REPLACE(reference_no,'SALE','INVOICE') as reference_no, biller, customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, attachment, return_id")
                ->from('sales')
    ...
    public function viewreceipt($id = null, $view = null, $save_bufffer = null)
    {
        $this->sma->checkPermissions();
        ...
        $this->load->view($this->theme . 'sales/viewreceipt', $this->data);
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $invoice_check_sale = $this->input->post('invoice_check_sale') ? $this->input->post('invoice_check_sale') : 0;
            ...
            $data = array('date' => $date,
                ...
                'due_date' => $due_date,
                'paid' => 0,
                'invoice_check_sale'    => $invoice_check_sale,
    ...
    public function add_delivery($id = null)
    {
        ...
        if ($this->sales_model->getDeliveryBySaleID($id)) {
            $this->session->set_flashdata('error', lang('delivery_already_added'));
            $this->sma->md();
        }
    ...
    public function add_payment($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $payment = array(
                ...
                'paid_by' => $this->input->post('paid_by'),
                'cheque_no' => $this->input->post('cheque_no'),
                'mpesa_no' => $this->input->post('mpesa_no'),
    ...
    public function suggestions()
    {
        ...
        if ($rows) {
            foreach ($rows as $row) {
                ...
                if($row->variant_id != "") {
                    $row->option = $row->variant_id;
                    
                } else {
                    $voptions = $this->sales_model->getProductOptionByID($row->id, $option_id);
					if($voptions)
                    	$row->option = $option_id;
                }
                //echo $row->option; exit;
                ...
                $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'category' => $row->category_id, 
                    'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options, 'variant_name' => $row->variant_name);
