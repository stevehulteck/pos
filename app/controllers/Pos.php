...
class Pos extends MY_Controller
{
	...
	public function change_poscheck($sale_id) {
		
		$this->pos_model->changeposchk($sale_id);
		$this->session->set_flashdata('message', "Pos Check Sale change successfully");
		redirect("pos/sales");
	}
    ...
    public function getSales($warehouse_id = NULL)
    {
		...
		$detail_link4 = anchor('pos/change_poscheck/$1', '<i class="fa fa-check-square-o"></i> ' . lang('Pos_Check_Sale'), 'data-id="$3" id="pcs"');
        ...
        $delete_link = "<a href='#' class='po' title='<b>" . lang("delete_sale") . "</b>' data-content=\"<p>"
            ...
            . lang('delete_sale') . "</a>";
        $action = '<div class="text-center"><div class="btn-group text-left" id="posactions">'
    ...
    <ul class="dropdown-menu pull-right" role="menu">
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, date, reference_no, biller, customer, (grand_total+rounding), paid, (grand_total-paid) as balance, payment_status, pos_check_sale as pos_check_sale, companies.email as cemail")
                ->from('sales')
            ...
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, date, reference_no, biller, customer, (grand_total+rounding), paid, (grand_total+rounding-paid) as balance, payment_status, pos_check_sale as pos_check_sale, companies.email as cemail")
                ->from('sales')
                ->join('companies', 'companies.id=sales.customer_id', 'left')
                ->group_by('sales.id');
        }
        ...
        $this->datatables->add_column("Actions", $action, "id, cemail, pos_check_sale")->unset_column('cemail')->unset_column('pos_check_sale');
    ...
    public function index($sid = NULL)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $pos_check_sale = $this->input->post('poschksale') ? $this->input->post('poschksale') : 0;
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if($item_option != "") {
                    $q = $this->db->get_where('product_variants', array('id' => $item_option), 1);
                    $pvariants = $q->row();
                    if($pvariants->sale_unit!="") {
                        $item_quantity = $item_quantity * $pvariants->sale_unit;
                    }
                }
    ...
    public function close_register($user_id = NULL)
    {
        ...
        //$this->form_validation->set_rules('total_mpesa', lang("Total_mpesa"), 'trim|required|numeric');
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $data = array(
                ...
                'total_mpesa'            => $this->input->post('total_mpesa'),
                ...
                'total_mpesa_submitted'  => $this->input->post('total_mpesa_submitted'),
        ...
        if ($this->form_validation->run() == TRUE && $this->pos_model->closeRegister($rid, $user_id, $data)) {
            ...
            $this->data['mpsales'] = $this->pos_model->getRegisterMpSales($register_open_time, $user_id);
    ...
    public function register_details()
    {
        ...
        $this->data['mpsales'] = $this->pos_model->getRegisterMpSales($register_open_time);
