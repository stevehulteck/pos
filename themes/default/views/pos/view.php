...
<div id="wrapper">
    ...
    <div id="receipt-data">
        <div class="text-center">
            ...
            <h3 style="text-transform:uppercase;"><?=$biller->company != '-' ? $biller->company : $biller->name;?> ltd</h3>
            <?php
                ...
                <?php
                	...
                	foreach ($rows as $row) {
                	    ...
                	    if ($row->item_discount != 0) {
                	        echo '<del>' . $this->sma->formatMoney($row->net_unit_price + ($row->item_discount / $row->unit_quantity) + ($row->item_tax / $row->unit_quantity)) . '</del> ';
                	    }
                	    echo $this->sma->formatMoney($row->net_unit_price + ($row->item_tax / $row->unit_quantity)) . '</td><td class="no-border border-bottom text-right">' . $this->sma->formatMoney($row->subtotal) . '</td></tr>';
                	    $r++;
                	}
                    if ($return_rows) {
            ...
            <?php
            	if ($payments) {
            	    ...
            	    foreach ($payments as $payment) {
                        ...
                        } elseif ($payment->paid_by == 'mpesa' && $payment->mpesa_no) {
            	            echo '<td>' . lang("paid_by") . ': ' . lang($payment->paid_by) . '</td>';
            	            echo '<td>' . lang("amount") . ': ' . $this->sma->formatMoney($payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '') . '</td>';
            	            echo '<td>' . lang("mpesa_no") . ': ' . $payment->mpesa_no . '</td>';    
