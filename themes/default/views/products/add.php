            ...
            <div class="col-lg-12">
                ...
                <div class="col-md-5">
                    ...
                    <div class="form-group standard">
                        ...
                        ?>
                        <?= form_dropdown('unit', $pu, set_value('unit'), 'class="form-control tip" id="unit" required="required" style="width:100%;"'); ?>
                    </div>
                    <div class="form-group standard">
                        <?= lang('default_sale_unit', 'default_sale_unit'); ?>
                        <?php $uopts[''] = lang('select_unit_first'); ?>
                        <?= form_dropdown('default_sale_unit', $uopts, '', 'class="form-control" id="default_sale_unit" style="width:100%;"'); ?>
                    </div>
                    <div class="form-group standard">
                        <?= lang('default_purchase_unit', 'default_purchase_unit'); ?>
                        <?= form_dropdown('default_purchase_unit', $uopts, '', 'class="form-control" id="default_purchase_unit" style="width:100%;"'); ?>
                    </div>
                    <div class="form-group standard">
                                    ...
                                    if ($this->input->post('attributes')) {
                                        ...
                                        for ($r = 0; $r <= $a; $r++) {
                                            if (isset($_POST['attr_name'][$r]) && (isset($_POST['attr_warehouse'][$r]) || isset($_POST['attr_quantity'][$r]))) {
                                                ...
                                                . '</td>'
                                                . '<td>'
                                                        . '<input type="hidden" name="attr_code[]" value="' . $_POST['attr_code'][$r] . '">'
                                                        . '<span>' . $_POST['attr_code'][$r] . '</span>'
                                                . '</td>'
                                                . '<td>'
                                                        . '<input type="hidden" name="attr_sale_unit[]" value="' . $_POST['attr_sale_unit'][$r] . '">'
                                                        . '<span>' . $_POST['attr_sale_unit'][$r] . '</span>'
                                                . '</td>'        
                                                . '<td class="code text-center">'
                                                        . '<input type="hidden" name="attr_warehouse[]" value="' . $_POST['attr_warehouse'][$r] . '">'
                                                        . '<input type="hidden" name="attr_wh_name[]" value="' . $_POST['attr_wh_name'][$r] . '">'
                                                        . '<span>' . $_POST['attr_wh_name'][$r] . '</span>'
                                                . '</td>'
                                        ...
                                        foreach ($product_options as $option) {
                                            ...
                                            . '</td>'
                                            . '<td class="attr">'
                                                    . '<input type="hidden" name="attr_code[]" value="' . $option->code . '">'
                                                    . '<span>' . $option->code . '</span>'
                                            . '</td>'
                                            . '<td>'
                                                    . '<input type="hidden" name="attr_sale_unit[]" value="' . $option->sale_unit . '">'
                                                    . '<span>' . $option->sale_unit . '</span>'
                                            . '</td>'        
                                            . '<td class="code text-center">'
                                            ...
                                            . '</td>'
                                            . '<td class="price text-right">'
                                                    . '<input type="hidden" name="attr_price[]" value="' . $this->sma->formatMoney($option->price) . '">'
                                                    . '<span>' . $this->sma->formatMoney($option->price) . '</span>'
                                                    . '</span>'
                                            . '</td>'
                                            . '<td class="text-center"><i class="fa fa-times delAttr"></i></td>'
                                            . '</tr>';
            ...
            for (var i in attrs) {
                if (attrs[i] !== '') {
                    <?php if( ! empty($warehouses)) {
                        foreach ($warehouses as $warehouse) {
                            echo '$(\'#attrTable\').show().append(\'<tr class="attr"><td><input type="hidden" name="attr_name[]" value="\' + attrs[i] + \'"><span>\' + attrs[i] + \'</span></td><td><input type="hidden" name="attr_code[]" value="\' + attrs[i] + \'"><span>\' + attrs[i] + \'</span></td><td><input type="hidden" name="attr_sale_unit[]" value="0"><span></span></td><td class="code text-center"><input type="hidden" name="attr_warehouse[]" value="'.$warehouse->id.'"><span>'.$warehouse->name.'</span></td><td class="price text-right"><input type="hidden" name="attr_price[]" value="0"><span>0</span></span></td><td class="text-center"><i class="fa fa-times delAttr"></i></td></tr>\');';
                        }
                    } else { ?>
                        $('#attrTable').show().append('<tr class="attr"><td><input type="hidden" name="attr_name[]" value="' + attrs[i] + '"><span>' + attrs[i] + '</span></td><td><input type="hidden" name="attr_code[]" value="' + attrs[i] + '"><span>' + attrs[i] + '</span></td><td><input type="hidden" name="attr_sale_unit[]" value="0"><span></span></td><td class="code text-center"><input type="hidden" name="attr_warehouse[]" value=""><span></span></td><td class="price text-right"><input type="hidden" name="attr_price[]" value="0"><span>0</span></span></td><td class="text-center"><i class="fa fa-times delAttr"></i></td></tr>');
                    <?php } ?>
                }
            }
        });
...
//});
        ...
        $(document).on('click', '.attr td:not(:last-child)', function () {
            ...
            $('#acode').val(row.children().eq(1).find('input').val());
            $('#asale_unit').val(row.children().eq(2).find('input').val());
            $('#awarehouse').select2("val", (row.children().eq(3).find('input').val()));
        ...
        $(document).on('click', '#updateAttr', function () {
            ...
            row.children().eq(1).html('<input type="hidden" name="attr_code[]" value="' + $('#acode').val() + '"><span>' + $('#acode').val() + '</span>');
            row.children().eq(2).html('<input type="hidden" name="attr_sale_unit[]" value="' + $('#asale_unit').val() + '"><span>' + $('#asale_unit').val() + '</span>');
            row.children().eq(3).html('<input type="hidden" name="attr_warehouse[]" value="' + wh + '"><input type="hidden" name="attr_wh_name[]" value="' + wh_name + '"><span>' + wh_name + '</span>');
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" id="pr_popover_content">
                <form class="form-horizontal" role="form">
                    <div class="form-group">
                        <label for="acode" class="col-sm-4 control-label"><?= lang('variant_code') ?></label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="acode">
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="asale_unit" class="col-sm-4 control-label"><?= lang('sale_unit') ?></label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="asale_unit">
                        </div>
                    </div>
                    <div class="form-group">
