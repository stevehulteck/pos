...
$('#posdiscount').focus(function () {
		...
		if(item.options !== false) {
			$.each(item.options, function () {
				if(this.id == item.row.option && this.price != 0 && this.price != '' && this.price != null) {
					unit_price = parseFloat(this.price);
				}
			});
		}
	...
	$(document).on('change', '#poption', function () {
		...
		if(item.options !== false) {
			$.each(item.options, function () {
				if(this.id == opt && this.price != 0 && this.price != '' && this.price != null) {
					$('#pprice').val((parseFloat(this.price)*parseFloat(base_quantity))).trigger('change');
				}
			});
		}
	});
...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		if(java_applet == 1) {
			...
			var styles = '<style>table, th, td { border-collapse:collapse; border-bottom: 1px solid #CCC; } .no-border { border: 0; } .bold { font-weight: bold; } @page { size: auto;  margin: 0mm; }</style>';
		...
		$.each(sortedItems, function () {
			...
			var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_price = item.row.price, item_qty = item.row.qty, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_serial = item.row.serial, item_variant_name = item.variant_name, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
			...
			if(item.options !== false) {
				$.each(item.options, function () {
					if(this.id == item.row.option && this.price != 0 && this.price != '' && this.price != null) {
						//item_price = (parseFloat(this.price)*parseFloat(item.row.base_quantity));
                                                item_price = (parseFloat(this.price));
			...
			if(java_applet == 1) {
				...
				if (pos_settings.item_order == 1 && print_cate) {
					...
					$("#bill-table").append(bprTh);
                                ...
                                if(typeof(item.options[0]) == 'undefined') {
                                    var bprTr = '<tr class="row_' + item_id + '" data-item-id="' + item_id + '"><td colspan="2" class="no-border">#'+(an-1)+' '+ item_name + '</td></tr>';
                                } else {
                                    var bprTr = '<tr class="row_' + item_id + '" data-item-id="' + item_id + '"><td colspan="2" class="no-border">#'+(an-1)+' '+ item_name + ' (' + item_variant_name + ')</td></tr>';
                                }
				bprTr += '<tr class="row_' + item_id + '" data-item-id="' + item_id + '"><td>(' + formatDecimal(item_qty) + ' x ' + (item_discount != 0 ? '<del>'+formatMoney(parseFloat(item_price) + parseFloat(pr_tax_val) + item_discount)+'</del>' : '') + formatMoney(parseFloat(item_price) + parseFloat(pr_tax_val))+ ')</td><td style="text-align:right;">'+ formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty))) +'</td></tr>';
                                if(typeof(item.options[0]) == 'undefined') {
                                    var oprTr = '<tr class="row_' + item_id + '" data-item-id="' + item_id + '"><td>#'+(an-1)+' ' + item_name + '</td><td>' + formatDecimal(item_qty) +'</td></tr>';
                                } else {
                                    var oprTr = '<tr class="row_' + item_id + '" data-item-id="' + item_id + '"><td>#'+(an-1)+' ' + item_name + ' (' + item_variant_name + ')</td><td>' + formatDecimal(item_qty) +'</td></tr>';
                                }
